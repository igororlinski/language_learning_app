...
// added to the schema fails here before it can reach a screen.
const mediaKinds = FIELD_KINDS.filter(isMediaKind);
const tables: [string, Record<MediaKind, unknown>][] = [
  ['katalog', MEDIA_DIRECTORIES],
  ['limit', MEDIA_LIMITS],
  ['typ MIME', MEDIA_MIME_TYPES],
  ['nazwa rodzaju', MEDIA_NOUNS],
  ['nazwa w dopelniaczu', MEDIA_NOUNS_GENITIVE],
  ['komunikat o braku pliku', MEDIA_MISSING_LABELS],
];

for (const [what, table] of tables) {
  check(
    `kazdy rodzaj ma ${what}`,
    mediaKinds.filter((kind) => table[kind] === undefined),
    []
  );
}
...
check('dzwiek do 5 MB', MEDIA_LIMITS.audio, 5 * 1024 * 1024);
check('obraz do 10 MB', MEDIA_LIMITS.image, 10 * 1024 * 1024);
check('wideo do 25 MB', MEDIA_LIMITS.video, 25 * 1024 * 1024);
...
check('wideo nad limitem tez', withinSizeLimit('video', MEDIA_LIMITS.video + 1), false);
// The same file may pass as a picture and fail as sound.
check('limity sa liczone osobno', withinSizeLimit('image', MEDIA_LIMITS.audio + 1), true);
check('i w druga strone', withinSizeLimit('audio', MEDIA_LIMITS.image), false);
...
check('limit wideo w komunikacie', formatBytes(MEDIA_LIMITS.video), '25,0 MB');
...
check(
  'wideo bez rozszerzenia dostaje mp4',
  storedFileName('video', 'klip', at, () => 0.5).endsWith('.mp4'),
  true
);
check(
