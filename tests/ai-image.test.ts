...
// Steps are 80% of what a picture costs — see the comment on STEPS.
...
check(
  ...
  await refusal(() => generateImage('   ', 'https://w.example.dev')),
  'empty-prompt'
);
check(
  'pusty prompt sceny tez nie',
  await refusal(() => generatePicture('   ', 'https://w.example.dev')),
  'empty-prompt'
);
